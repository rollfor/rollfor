# RollFor - SR Provider (raidres.top)

The raidres import format for
...
Drop `RollForRaidRes` into `Interface/AddOns`, next to `RollForSoftRes` and `RollFor`.

`RollForSoftResIt` can be installed at the same time -- that is the point of the split.
Both appear in the import window's Provider dropdown and you pick per import. (Before the
split the two addons each carried a whole copy of the soft-res addon and were mutually
exclusive.)

## Importing

1. Open your raid on [raidres.top](https://raidres.top).
2. Copy the raid's import string.
3. `/sr` in game, pick **raidres.top** in the Provider dropdown, paste the string, and hit
   Import.

A raidres export is plain base64 over JSON. A softres.it string is base64 over *zlib*,
which this provider does not read -- pasting one with raidres.top selected reports that the
data could not be loaded, and naming the provider in that message is how you know which
...
again, and redo any name overrides.

## Options

RollFor's options window gets a page for this addon, with the switch to turn it off. Turned
off, raidres.top leaves the Provider dropdown at the next UI reload. A list already imported
