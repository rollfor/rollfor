...
-- Enabled checkbox and reports its version; and with `RollForSoftRes`, so "softres.it" is in
-- the import window's Provider dropdown. Decoder_test covers the format. This covers the
...
require( "Decoder" )
local softres_it = require( "RollForSoftResIt" )
...
local function toc_field( field )
  local file = assert( io.open( "../src/RollForSoftResIt.toc" ) )
  local result
...
function ExtensionSpec:should_have_registered_itself_on_load()
  ...
  eq( found.title, "SR Provider (softres)" )
...
function ProviderSpec:should_hand_its_decoder_to_the_library_on_enable()
  ...
  softres_it.on_enable()

  local provider = assert( registered_provider() )
  eq( provider.title, "softres.it" )
  eq( provider.decode, RollForSoftResIt.Decoder.decode )
end
