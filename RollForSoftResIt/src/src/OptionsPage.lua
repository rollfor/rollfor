...
--
-- It can describe a website again, unlike RollFor - SoftRes's page: a provider is exactly
-- the piece that knows which one.
local SUMMARY = string.format(
  "A SoftRes data provider using %s's Gargul export format.", hl( "softres.it" )
)
...
function M.new( ctx, parent )
  ...
  local function create_popup()
    local result = ctx.popup_builder()
        :name( "RollForSoftResItOptionsPage" )
        :parent( parent )
        :point( {
          point = "TOPLEFT",
