...
run_all_tests() {
  ...
  local files file
  local failed=()
  mapfile -t files < <(find . -name "*_test.lua" | sort)
  ...
  echo >&2
  if [[ ${#failed[@]} -eq 0 ]]; then
    echo -e "\033[1;32mAll ${#files[@]} test files passed.\033[0m" >&2
    return 0
  fi

  echo -e "\033[1;31m${#failed[@]} of ${#files[@]} test files failed:\033[0m" >&2
  printf "  %s\n" "${failed[@]}" >&2
  return 1
}
