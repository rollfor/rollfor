...
if sr.SoftResCheck then return end
...
---@param store table -- the SoftResStore, asked how old its data is
---@param event_bus EventBus -- told when /src has run, so others can report on the same data
function M.new( softres, group_roster, name_matcher, ace_timer, absent_softres, store, event_bus )
  ...
  local function check_softres( silent )
    local timestamp = store.get_import_timestamp()
    ...
    if silent ~= true then sr.NameMatchReport.report( name_matcher ) end
  ...
  local function show_softres( link, retry )
    ...
    sr.NameMatchReport.report( name_matcher )
  ...
  -- Said out loud so anything else with something to report about this data can add its
  -- lines after ours -- RollForBtSrLimitCheck's budget check does exactly that. It is the
  -- command that is this addon's, not the subject: what else is worth checking about a
  -- soft-res list is not a question this file should be answering.
  local function on_check_command()
    local result = check_softres()
    -- The provider that produced the list being checked, so a listener can tell one
    -- source's checks from another's. Asked of the store rather than held here: the store
    -- is what knows, and it is already an argument.
    event_bus.notify( "softres_checked", { source = store.get_provider() } )
