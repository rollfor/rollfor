-- A test double, so putting fields on the frames it hands back is what it is for rather
-- than a mistake it makes. The same declaration the shared harness opens with.
...
function M.new()
  ...
  local builder = {
    ...
    build = function()
      ---@type Popup
      return {
        -- Required on Popup, and read by resize() as the horizontal margin it adds to the
        -- widest line. Zero here: nothing in these suites measures a popup's width.
        side_margin = 0,
