...
-- lookups.
--
-- EXTENSION: and it matters far more here than it does for Nether Vortex. This addon ships
-- `src/SoftResCheck.lua`, `src/SoftResGui.lua`, `src/NameAutoMatcher.lua` and six more
-- files whose names core also uses, so `require( "src/SoftResCheck" )` is core's and must
-- stay core's while core still has its built-in. This addon's own modules are therefore
-- required by bare name -- `require( "SoftResCheck" )` -- which only `../src/?.lua`
-- resolves, and it is listed last so it can never shadow anything.
package.path = "./?.lua;" .. package.path ..
    ";../../RollFor/src/?.lua;../../RollFor/src/libs/?.lua;../../RollFor/src/libs/LibStub/?.lua;../src/?.lua;../../?.lua;../src/src/?.lua"
...
-- EXTENSION: every UIDropDownMenu built since the API was mocked, newest last, and the
-- buttons the most recent one produced. See mock_wow_api.
local m_dropdowns = {}
local m_dropdown_buttons = {}
...
function M.mock_wow_api()
  ...
  -- EXTENSION: the import window carries a Provider dropdown, which GuiElements.dropdown
  -- builds out of UIDropDownMenuTemplate. Core's own harness has no test that opens one,
  -- so these are not in the copy this file was taken from.
  --
  -- The initializer is kept rather than run: the client calls it when the menu is opened,
  -- and M.dropdown_buttons() below is a test doing the same thing. That way a test asserts
  -- what the menu actually renders instead of what the caller passed to SetOptions.
  m_dropdowns = {}
  M.modules().api.UIDropDownMenu_SetWidth = function() end
  M.modules().api.UIDropDownMenu_Initialize = function( dropdown, initialize )
    dropdown.initialize = initialize
    table.insert( m_dropdowns, dropdown )
  end
  M.modules().api.UIDropDownMenu_CreateInfo = function() return {} end
  M.modules().api.UIDropDownMenu_AddButton = function( info ) table.insert( m_dropdown_buttons, info ) end
  M.modules().api.UIDropDownMenu_SetSelectedValue = function( dropdown, value ) dropdown.selected_value = value end
  M.modules().api.UIDropDownMenu_SetText = function( dropdown, text ) dropdown.selected_text = text end
  ...
  M.modules().api.CreateFrame = function( _, frame_name )
    local frame = {
      ...
      -- EXTENSION: recorded rather than swallowed. The import window disables its Import
      -- button when no provider is registered, and a test has no other way to see that.
      ...
      -- EXTENSION: also recorded. The no-provider state makes the editbox inert without
      -- clearing it, which is the half of the behaviour the text alone cannot show.
      EnableMouse = function( self, value ) self.mouse_enabled = value ~= false end,
      EnableKeyboard = function( self, value ) self.keyboard_enabled = value ~= false end,
      -- EXTENSION: the soft-res list scrolls, and FrameBuilder turns the wheel on for it.
      ...
      CreateFontString = function()
        return {
          -- EXTENSION: visibility recorded. The "no providers registered" message is a
          -- font string that is shown or hidden, and nothing else about it changes.
          visible = true,
          Hide = function( self ) self.visible = false end,
          Show = function( self ) self.visible = true end,
          IsVisible = function( self ) return self.visible and true or false end,
          ClearAllPoints = function() end,
          SetPoint = function() end,
...
-- EXTENSION: what the most recently built dropdown puts in its menu, by opening it. Each
-- entry is the client's own info table: `text`, `value`, `checked` and `func`, which is
-- what clicking the row runs.
---@return table[]
function M.dropdown_buttons()
  local dropdown = m_dropdowns[ #m_dropdowns ]
  if not dropdown or not dropdown.initialize then return {} end

  m_dropdown_buttons = {}
  dropdown.initialize()

  return m_dropdown_buttons
end

-- EXTENSION: picks an option out of that menu the way a user does -- the button's own
-- func, which is what tells the widget and its owner that the selection changed.
---@param value any
function M.click_dropdown_option( value )
  for _, info in ipairs( M.dropdown_buttons() ) do
  ...
  error( string.format( "No dropdown option with value %s.", tostring( value ) ), 2 )
end
...
-- which is exactly where the client puts them. In TOC order, and by bare name: see the
-- package.path note at the top of this file for why these are not `src/...`.
...
function M.load_extension()
  ...
  -- Core modules this addon's files reach for at file scope, plus the registry and chain
  -- it registers into. Named here rather than assumed, because a test that builds only
  -- part of RollFor still has to be able to load the extension.
  require( "src/modules" )
  ...
  -- Core's SoftRes, which after the extraction is types + softres_item_data + null(). This
  -- addon's store builds its item data with it, so it is a real dependency and not just
  -- load-order noise.
  require( "src/SoftRes" )
  require( "src/Chain" )
  ...
  require( "SoftResStore" )
  require( "SoftResAwardedLootDecorator" )
  require( "SoftResPresentPlayersDecorator" )
  require( "SoftResAbsentPlayersDecorator" )
  require( "SoftResMatchedNameDecorator" )
  require( "SoftResDisabledEntries" )
  require( "SoftResDisabledEntriesDecorator" )
  require( "NameAutoMatcher" )
  require( "NameManualMatcher" )
  require( "NameMatchReport" )
  require( "SoftResCheck" )
  require( "SoftResGui" )
  require( "SoftResListContentTransformer" )
  require( "SoftResListWidgets" )
  require( "SoftResListFrame" )
  require( "Simulation" )
  require( "Minimap" )
  require( "OptionsPage" )
...
function M.load_roll_for()
  ...
  -- produces from `## Dependencies: RollFor`. Requiring "RollForSoftRes" here
...
function M.import_soft_res( data )
  ...
  -- EXTENSION: the import belongs to whoever owns the source. With this addon loaded that
  -- is us, and core has no store left to import into.
  if source and source.id == "softres" then
    RollForSoftRes.main.import_softres_data( data )
    return rf
  end
...
function M.create_softres_data( ... )
  ...
  for i = 1, #items do
    ...
    if item.soft_res then
      ...
      -- EXTENSION: this builds a *provider's* document, not this addon's data, so it carries
      -- what a provider's document carries -- raidres puts a roll bonus on every item entry.
      -- Nothing in this addon reads it; RollForSrPlusRaidres does, off softres_imported.
      -- Absent unless a test asks for it, so every existing case builds what it always did.
...
-- EXTENSION: `sr_plus` is the per-reservation roll bonus raidres emits. One entry is one
-- reservation, so two entries for the same player and item are two rolls -- and may carry
-- two different bonuses, which is what RollForSrPlusRaidres' highest-wins rule is about.
---@param player string
...
-- EXTENSION: the window asks which provider a string came from before it will decode it,
-- so this picks one first. Named explicitly when a case has more than one registered;
-- omitted, it takes whatever the dropdown is already showing, which with a single provider
-- installed is that provider.
---@param fixture_name string
---@param provider_id string?
function M.import_softres_via_gui( fixture_name, provider_id )
  local sr_data = M.read_file( fixture_name )
  ---@diagnostic disable-next-line: undefined-global
  local sr_frame = _G[ "RollForSoftResImportFrame" ]

  if provider_id then M.click_dropdown_option( provider_id ) end
