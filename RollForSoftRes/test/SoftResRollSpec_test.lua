...
function ThreeIdenticalItemsOneSrSpec:should_announce_three_items_then_rf_command()
  -- Given
  local loot_facade, chat = mock_loot_facade(), mock_chat()
  local c, r, rw = chat.console, chat.raid, chat.raid_warning
  ...
  rf.rolling_popup.should_display(
    ...
    text( "Psikutas soft-ressed this item.", 11 ),
    individual_award_button,
    text( "Obszczymucha wins the main-spec roll with 80.", 8 ),
    individual_award_button,
    text( "Jimmy wins the main-spec roll with 70.", 8 ),
    individual_award_button,
    buttons( "RaidRoll", "AwardOther", "Close" )
  )
end
...
function ThreeIdenticalItemsTwoSrSpec:should_announce_three_items_then_rf_command()
  -- Given
  local loot_facade, chat = mock_loot_facade(), mock_chat()
  local c, r, rw = chat.console, chat.raid, chat.raid_warning
  local item, item2, item3 = i( "Bag", 69 ), i( "Bag", 69 ), i( "Bag", 69 )
...
function AwardedLootSpec:should_award_the_sr_item_then_roll_for_the_rest()
  ...
  -- When (the two identical items drop)
  loot_facade.notify( "LootOpened", item, item2 )
  master_loot.flush()
  ...
  -- Then (the announcement every award gets: /award goes through the same callback master loot
  -- and trading do, so the pending list and the winner tracker hear it too)
  c( "RollFor: Psikutas received [Bag]." )
