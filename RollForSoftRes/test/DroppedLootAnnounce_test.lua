...
local utils = require( "RollForSoftRes/test/utils" )
utils.mock_wow_api()
...
require( "src/Types" )
-- The drop announcement annotates a name with what a roll modifier would add to it, so it
-- reaches for RollingLogicUtils -- which resolves its order through Ordering. Neither is
-- optional: with no modifier registered the annotation is empty, but the ask still happens.
require( "src/Ordering" )
require( "src/RollingLogicUtils" )
-- Core's SoftRes is still here for softres_item_data and the shared types; the store
-- these specs build on is this addon's.
require( "src/SoftRes" )
